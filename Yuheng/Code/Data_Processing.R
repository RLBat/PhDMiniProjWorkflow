...
#Species_History <- read.csv("My_Workflow/Data/Species_History.csv", stringsAsFactors = T)
#Species_Data <- read.csv("My_Workflow/Data/Species_Data.csv", stringsAsFactors = F)
...
Assess_Clean <- function(Species_History){
  ...
  Species_History$category<-droplevels(as.factor(Species_History$category))
...
Assign_known_tags <- function(Cat_Changes, Species_History){
  ...
  Species_History$Verified <- as.character(NA)
