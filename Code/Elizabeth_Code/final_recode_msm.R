...
Change1 <-ceiling(NGmatrix_full[2,1]/summary[8,8] * sum(df$new==0)) #DDsbLC
Change3 <-ceiling(NGmatrix_full[2,3]/summary[8,2] * sum(df$new==6)) #NTsbLC
Change4 <-ceiling(NGmatrix_full[3,1]/summary[8,8] * sum(df$new==0)) #DDsbNT
Change5 <-ceiling(NGmatrix_full[3,2]/summary[8,1] * sum(df$new==2)) #LCsbNT
Change7 <-ceiling(NGmatrix_full[3,4]/summary[8,3] * sum(df$new==10)) #VUsbNT
Change8 <-ceiling(NGmatrix_full[4,1]/summary[8,8] * sum(df$new==0)) #DDsbVU
Change9 <-ceiling(NGmatrix_full[4,3]/summary[8,2] * sum(df$new==6)) #NTsbVU
Change11 <-ceiling(NGmatrix_full[4,5]/summary[8,4] * sum(df$new==14)) #ENsbVU
Change12 <-ceiling(NGmatrix_full[5,1]/summary[8,8] * sum(df$new==0)) #DDsbEN
Change13 <-ceiling(NGmatrix_full[5,4]/summary[8,3] * sum(df$new==10)) #VUsbEN
Change15 <-ceiling(NGmatrix_full[5,6]/summary[8,5] * sum(df$new==18)) #CRsbEN
Change16 <-ceiling(NGmatrix_full[6,1]/summary[8,8] * sum(df$new==0)) #DDsbCR
Change17 <-ceiling(NGmatrix_full[6,5]/summary[8,4] * sum(df$new==14)) #ENsbCR
Change19 <-ceiling(NGmatrix_full[6,7]/summary[8,6] * sum(df$new==23)) #EWsbCR
Change20 <-ceiling(NGmatrix_full[6,8]/summary[8,7] * sum(df$new==26)) #EXsbCR
Change21 <-ceiling(NGmatrix_full[7,1]/summary[8,8] * sum(df$new==0)) #DDsbEW
Change22 <-ceiling(NGmatrix_full[7,6]/summary[8,5] * sum(df$new==18)) #CRsbEW
Change24 <-ceiling(NGmatrix_full[8,1]/summary[8,8] * sum(df$new==0)) #DDsbEX
Change25 <-ceiling(NGmatrix_full[8,6]/summary[8,5] * sum(df$new==18)) #CRsbEX
...
df <- df %>% group_by(name) %>% filter(n()>1) #  106083 obs
...
# Using the TSFO != 0 above, there will remain an issue where 7 is the first in a series of listings for a species.  The following will recode these as the next entry in the ex column (ie having run the piece of code above) 
#
#for (i in 1:nrow(df)){
#  
#  
#  if (df[i, ]$ex==7 & df[i, ]$TSFO ==0){
#    
#    df[i,]$ex <- df[i+1, ]$ex
#  }
#}
...
# There will remain a small number of oddities ie 5, 7, 5, 5 where it was originally 5, 7, 7, 5 (Latonia nigriventer) - running this modified piece of code will correct them
# NB needs to run twice as it only corrects one 7 to another state at a time (generally the  statetable will still show one or two species reverting from state 7 to previous states, so if we had 5, 7, 7, 7, 5 for example).
...
# third and subsequent runs through
for (i in 1:nrow(df)){
...
dflose <- dplyr::filter(df, msm==8) # 767 entries

df <- dplyr::anti_join(df, dflose) # 105316 obs

# also need to remove any species which are now left with only one entry

df <- df %>% group_by(name) %>% filter(n()>1) # 105085 obs

# now look at the data using a statetable in msm

statetable.msm(msm, name, data=df)
# 374 7 to 7 transitions remain
...
dflose2 <- dplyr::filter(df, msm==8) # 260 entries

df <- dplyr::anti_join(df, dflose2) 

df <- df %>% group_by(name) %>% filter(n()>1) #104715

# check statetable again

statetable.msm(msm, name, data=df)
# remove final 114 trailing 7s
...
dflose3 <- dplyr::filter(df, msm==8) #  114 entries

df <- dplyr::anti_join(df, dflose3) # 104601

df <- df %>% group_by(name) %>% filter(n()>1) #104490
...
write.csv(df, "../Data/msm.csv", row.names = FALSE)
...
write.csv(df, "../Data/msm_noEW.csv", row.names=FALSE)
